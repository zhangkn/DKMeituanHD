...
@property (nonatomic,strong) id<DKHomeDropdownViewData> model;//视图对应的模型，是视图提供给外界的接口
+ (instancetype) tableViewCellWithTableView:(UITableView *) tableView;//使用类方法加载xib
/**
 通过数据模型设置视图内容，可以让视图控制器不需要了解视图的细节
 */
+ (instancetype) tableVieCellWithModel:(id<DKHomeDropdownViewData> ) model tableView:(UITableView *)tableView;//使用类方法加载xib,参数用于视图的数据装配
